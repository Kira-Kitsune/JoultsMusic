...
export default new Command({
	...
	options: [
		...
		{
			name: "your-last-song",
			description: "Remove the last song in the queue that was added by you.",
			type: ApplicationCommandOptionType.Subcommand,
			options: [],
		},
	],
			...
			case "position":
				{
					...
					// biome-ignore lint/correctness/noSelfAssign: need to
					queue.songs = queue.songs;
					const removedSongUrl =
						removedSong[0].stream.playFromSource === true
							? removedSong[0].stream.url
							: // @ts-ignore
								removedSong[0].stream.song.stream.url;
					embed.setDescription(
						`🗑️ Removed [${removedSong[0].name}](${removedSongUrl}) from queue`,
					);
				}
				break;
			case "by-name":
				{
					...
					const songListNames = Array.from(queue.songs, ({ name }) => name);
					...
					// biome-ignore lint/correctness/noSelfAssign: need to
					queue.songs = queue.songs;
					const removedSongNameUrl =
						removedSongName[0].stream.playFromSource === true
							? removedSongName[0].stream.url
							: // @ts-ignore
								removedSong[0].stream.song.stream.url;
					embed.setDescription(
						`🗑️ Removed [${removedSongName[0].name}](${removedSongNameUrl}) from queue`,
					);
				}
				break;
			case "your-last-song":
				{
					...
					const removedSongLast = queue.songs.splice(matchedIdIndex, 1);
					// biome-ignore lint/correctness/noSelfAssign: need to
					queue.songs = queue.songs;
					const removedSongLastUrl =
						removedSongLast[0].stream.playFromSource === true
							? removedSongLast[0].stream.url
							: // @ts-ignore
								removedSongLast[0].stream.song.stream.url;

					embed.setDescription(
						`🗑️ Removed [${removedSongLast[0].name}](${removedSongLastUrl}) from queue`,
					);
				}
				break;
		}
