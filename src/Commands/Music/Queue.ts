...
export default new Command({
	...
	run: async ({ client, interaction }) => {
		...
		const generateEmbed = (start: number) => {
			...
			if (songs[0]) {
				const songUrl =
					songs[0].stream.playFromSource === true
						? songs[0].stream.url
						: // @ts-ignore
							songs[0].stream.song.stream.url;

				embed.addFields([
					{
						name: "Current",
						value: `[${songs[0].name}](${songUrl})\nDuration: ${queue.formattedCurrentTime} / ${queue.songs[0].formattedDuration}`,
					},
					{
			...
			current.forEach((song, i) => {
				...
				embed.addFields({
					name: `Track ${start + i + 1} - Duration: ${duration}`,
			...
			.then((message) => {
				...
				collector.on("collect", (ButtonInteraction) => {
					...
					if (id === "back-queue") {
						...
						if (currentIndex + amountpp < songs.length - 1) {
							currentIndex =
								Math.floor((songs.length - 2) / amountpp) * amountpp;
							ButtonInteraction.update({
								embeds: [generateEmbed(currentIndex)],
								components: [row],
							});
